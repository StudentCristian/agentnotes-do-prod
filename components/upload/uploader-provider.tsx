...
export function UploaderProvider<TOptions = unknown>({
  ...
  const runUploads = React.useCallback(
    async (filesToUpload: FileState[], options?: TOptions) => {
      await Promise.all(
        filesToUpload.map(async (fileState) => {
          const abortController = new AbortController();
          abortControllersRef.current.set(fileState.key, abortController);
    ...
    [onUploadCompleted, updateFileState, uploadFn],
  );
  ...
  const uploadFiles = React.useCallback(
    async (keysToUpload?: string[], options?: TOptions) => {
      ...
      await runUploads(filesToUpload, options);
    },
    [fileStates, runUploads],
  );
  ...
  const addFiles = React.useCallback(
    (files: File[]) => {
      ...
      if (autoUpload) {
        void runUploads(newFileStates);
      }
    },
    [autoUpload, onFileAdded, runUploads],
  );
